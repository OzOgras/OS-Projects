...
static double *sqrts;
static double *lastPage;  // the page we have to munmap
static double *currPage;  // the page we are currently on (mmap)
...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
   ...
   // What: Unmap last page whenever new page is faulted in
   // map in a page at the faulting address, unmap a previous page to stay within the physical memory limit,
   // and initialize the new page with the correct square root values.
	
   // Get the adress of fault
   uintptr_t fault_addr = (uintptr_t)si->si_addr;
   
   // unmap last page and return error message if unsuccessful.
   // check if lasPage is not zero because the alignment has to be greater than 0
   if (lastPage > 0 && (munmap(lastPage ,page_size) == -1)) {
      fprintf(stderr, "Couldn't munmap() region for sqrt table; %s\n",
            strerror(errno));
      exit(EXIT_FAILURE);
   }

   // get the alignment for the page we need to map
   currPage = (double*)align_down(fault_addr, page_size);

   // map in a page at the faulting address which is currPage
   // set the Protection to write to make the new memory allocation writable
   currPage = mmap(currPage, page_size, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

   // return an error message if mapping failed.
   if (currPage == MAP_FAILED) {
      fprintf(stderr, "Couldn't mmap() region for sqrt table; %s\n",
            strerror(errno));
      exit(EXIT_FAILURE);
   }

   // initialze new page with new square root values
   calculate_sqrts((double *)currPage,((double *)currPage-sqrts),((int)page_size/8));
   lastPage = currPage;        // update the lastPage to currPage 
   return;
}
